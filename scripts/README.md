...
2. `deploy-infra.sh` — create/update the Azure app/agent resource group after `what-if` review; by default it reuses the existing Databricks workspace configured in `infra/main.demo.bicepparam`, connects Foundry to workspace-based Application Insights for traces, and can optionally add the per-band frontend App Services when `deployControlledFrontend` / `deployDeclarativeFrontend` / `deployOpenEndedFrontend = true`.
