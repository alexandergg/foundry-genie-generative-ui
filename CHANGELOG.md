...
- Open-ended band demo (`apps/open-web` + `apps/open-agent`, ports 3002/8125), mirroring the course L5: `openGenerativeUI: true` (the agent writes sandboxed HTML/CSS/JS via the client-registered `generateSandboxedUi` tool, rendered live while streaming) plus `mcpApps` with the public Excalidraw server (the runtime discovers the MCP tools, executes the call, and embeds the whiteboard in chat). The agent binds all injected tools and streams. Requires only `FOUNDRY_MODEL_ENDPOINT` — no Databricks (Excalidraw needs internet).
