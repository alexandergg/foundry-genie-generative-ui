...
def test_build_trace_callbacks_empty_when_disabled() -> None:
    assert build_trace_callbacks("risk-exposure-ag-ui-hosted", env={}) == []


def test_build_trace_callbacks_returns_tracer_when_enabled() -> None:
    from langchain_azure_ai.callbacks.tracers import AzureAIOpenTelemetryTracer

    callbacks = build_trace_callbacks("risk-exposure-ag-ui-hosted", env={"APPLICATIONINSIGHTS_CONNECTION_STRING": _FAKE_CONN})
